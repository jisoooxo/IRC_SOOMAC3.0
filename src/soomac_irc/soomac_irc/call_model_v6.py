...
from soomac_irc.order_v6 import SAUCES, NOODLE_TYPES, ALL_TOPPINGS, AMOUNTS, TOPPING_AMOUNTS, RESTRICTION_REASONS, RESTRICTION_CATEGORY_ITEMS, SECTION_ORDER, missing_requirements
...
def parse_tool_call(raw: str) -> dict | None:
    ...
    # 메뉴 정본 검증은 order_v6에서 한 번 더 하므로 여기서는 Tool별 모양을 먼저 막음
    try:
        parsed = json.loads(raw)
    except (json.JSONDecodeError, TypeError):
        return None
    ...
    # restriction은 여러 개가 들어올 수 있으며 값 검증은 order_v6가 담당한다.
