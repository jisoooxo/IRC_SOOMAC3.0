...
from soomac_irc.agent_v4 import build_graph
from soomac_irc.call_model_v4 import load_model, make_call_model, make_call_vlm, make_generate_reply
...
class LLMNode(Node):
    def __init__(self):
        super().__init__("soomac_llm_node_v4")
    ...
    def _build_turn_reply(self, user_text: str, state_before: dict, result: dict) -> str | None:
        # Python 정책 문장이 있으면 자유 Reply보다 항상 우선한다.
        transaction = result.get("transaction")
        policy_reply = result.get("policy_reply")
        ...
        # 주문 변경이 없는 질문만 Tool adapter를 끄고 자유 답변한다.
        if action == "respond":
            return self.generate_reply(state_before, user_text)
