...
from order_v3 import new_order, missing_required, next_section, build_section_plan, validate_delta, apply_delta, enforce_constraints, SECTION_ORDER, SECTION_LABELS, VEGGIES, MEATS, EXTRAS, EARLY_TOPPINGS, CONSTRAINT_BLOCKS
from call_model_v3 import load_model, make_call_model, make_generate_reply, make_call_vlm, SCENE_SYSTEM
from agent_v3 import build_graph, HISTORY_TURNS
...
RUNTIME_EVENT_LOG_PATH = Path.home() / "soomac_runtime_logs" / "runtime_events.jsonl" # ROS 용도 ㅇㅇ
...
IMAGE_TOPIC_TYPE = CompressedImage # Image
...
class LLMNode(Node):
    def __init__(self):
        super().__init__("soomac_llm_node_v3")
        ...
        self.section = "noodle" # noodle → veggie → meat → extra → lid → sauce
        ...
        while True:
            ...
            try:
                if mode == "start":
                    ...
                    elif self.conversation_started or self.active_task is not None or self.waiting_pick:
                    ...
                    if self.section != "sauce" or self.active_task is None: 
                    ...
                    elif ENABLE_VLM and not self.vlm_confirmed:
                        self.get_logger().warning("소스 VLM PASS 전이라 /llm/reset을 무시함")
                        ...
                        self.reply_pub.publish(String(data="소스까지 모두 담았어요. 이용해 주셔서 감사합니다."))
                        self._finish_order()
 ...
 # CompressedImage.data에는 JPEG/PNG 파일 전체가 압축된 바이트로 들어온다.
        if isinstance(message, CompressedImage):
            if not message.data:
                return None

            try:
                with PILImage.open(BytesIO(message.data)) as compressed_image:
                    return compressed_image.convert("RGB").copy()

            except (OSError, ValueError) as exc:
                self.get_logger().warning(
                    f"압축 카메라 이미지를 PIL로 바꾸지 못함 : "
                    f"format={message.format}, error={exc}"
                )
                return None

        # 아래의 encoding·width·height·step 변환은 raw Image에만 적용한다.
        if not isinstance(message, Image):
            self.get_logger().warning(
                f"지원하지 않는 카메라 메시지 타입 : {type(message).__name__}"
            )
            return None
    ...
    def _prepare_order(self):
        ...
        self.runtime_turn_index = 0 # 이건 llm용
        self.runtime_event_index = 0 # 이건 ros 이벤트용(토픽 같은거 ㅇㅇ)
    ...
    def _section_items(self):
        ...
        # 식이 제약은 order_v3.enforce_constraints()에서 별도로 검사한다.
    ...
    def _section_select_prompt(self):
        ...
        if self.section == "noodle":
            ...
            return "재료를 모두 담았어요. 이제 뚜껑을 닫을게요."

        elif self.section == "sauce":
            return "마지막으로 고르신 소스를 올릴게요."
    ...
    def _confirm_current_section(self, reply):
        ...
        # v3에서는 항상 로봇을 기다리므로 실제로는 tasks가 없을 때만 도달한다.
    ...
    def _commit_section(self, reply, skipped=False):
        ...
        # 추가 재료 단계가 끝나면 손님에게 다시 묻지 않고 뚜껑 작업을 시작한다.

        elif self.section == "lid":
            self.recommendation_state = {}
            return self._confirm_current_section(f"{reply} 이제 뚜껑을 닫을게요.")
        """
        extra 완료
        → “이제 뚜껑을 닫을게요.”
        → cover plan 발행
    ...
    def _process_vlm_confirm(self, should_confirm: bool):
        ...
        # cover를 제외한 모든 재료는 현재 작업 하나만 VLM으로 판정한다.
    ...
    def _process_next(self, result_code: int):
        ...
        # sauce는 마지막 작업이므로 VLM 판정과 home 복귀 뒤 main의 /llm/reset으로 완료한다.
        if self.section == "sauce":
            self.get_logger().warning("마지막 소스 작업 후 main의 /llm/reset을 기다리는 중")
            return

        # cover는 VLM 판정 없이 /llm/next=3으로 완료하고 다음 sauce 작업을 시작한다.
        needs_vlm_confirm = ENABLE_VLM and self.section != "lid"
    ...
    def _record_runtime_event(self, event, payload=None):
        ...
        try:
            ...
            event_row = {
                ...
                "state": {
                    ...
                    "waiting_pick": self.waiting_pick,
                    "vlm_confirmed": self.vlm_confirmed,
                    "order_finished": self.order_finished,
                },
            }

            with RUNTIME_EVENT_LOG_PATH.open("a", encoding="utf-8") as log_file:
                log_file.write(json.dumps(event_row, ensure_ascii=False) + "\n")

        except Exception as exc:
            self.get_logger().warning(f"런타임 이벤트 로그 저장 실패 : {exc}")
        ...
        if ENABLE_RUNTIME_LOG and isinstance(self, LLMNode):
            try:
                ...
                log_row = {
                    ...
                    "prompt_version": "tool_v3_2026-09-07_lid_sauce",
