...
from soomac_irc.order_v4 import EXTRAS, MEATS, NOODLE_TYPES, SAUCES, SECTION_ORDER, VEGGIES, apply_menu_changes, apply_restriction_changes, missing_requirements, order_restriction_conflicts, physical_section_for, validate_menu_changes, validate_recommended_order, validate_restriction_changes

class ToolCall(TypedDict):
    # call_model_v4가 만든 한 턴의 최소 Tool 계약
    call_id: str
    name: str
    changes: dict
...
def validate_transaction(state: AgentState) -> dict:
    ...
    if action == "refuse_section":
        ...
        if section == "veggie":
            section_items = VEGGIES
        elif section == "meat":
            section_items = MEATS
        elif section == "extra":
            section_items = EXTRAS
        else:
            section_items = None
        ...
        # 아직 담지 않은 현재 section 재료만 주문에서 제거
        for item in section_items:
        ...
        if section in confirmed_sections:
            confirmed_sections.remove(section)

        # 기존 추천안이 있었다면 stale proposal을 사용하지 않음
        recommendation_after = {
            "phase": "idle",
            "confirmed_sections": copy.deepcopy(confirmed_sections),
        }

        transaction["section_skip"] = {
            "section": section,
            "reason": reason,
            "needs_confirmation": needs_confirmation,
            "applied": not needs_confirmation,
        }
        transaction["order_after"] = copy.deepcopy(order_after)
