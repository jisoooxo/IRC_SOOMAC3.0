...
from call_model_v3 import TOOL_SYSTEM
from order_v3 import SECTION_ORDER, apply_delta, enforce_constraints, missing_required, validate_delta, build_section_plan
...
def build_graph(call_model, generate_reply):
    ...
    def choose_action(state: AgentState) -> dict:
        ...
        if section == "noodle":
            ...
            section_rule = "현재는 선택한 소스를 투입하기 전에 뚜껑을 닫는 물리 작업 단계이다. 새로운 주문값을 선택하는 단계가 아니다."

        elif section == "sauce":
            section_rule = "현재는 뚜껑을 닫은 뒤 선택한 소스를 마지막으로 투입하는 물리 작업 단계이다. 새로운 주문값을 선택하는 단계가 아니다."
    ...
    def execute_action(state: AgentState) -> dict:
        ...
        if tool_call is not None:
            ...
            if action in ("set_order", "set_order_and_confirm"):
                ...
                # call_model_v3.parse_tool_call()을 통과한 새 Tool 전용 값이다.
