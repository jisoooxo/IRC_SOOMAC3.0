...
def _confirmation_block_reason(intent: bool | None) -> str:
    return (
        "explicit_rejection"
        if intent is False
...
def validate_transaction(state: AgentState) -> dict:
    ...
    if action == "confirm_section" and recommendation_after.get("phase") == "confirming":
        ...
        if confirmation_intent is not True:
            transaction["confirm_validation"] = {
                ...
                "allowed": False,
                "missing": {},
                "reason": _confirmation_block_reason(confirmation_intent),
            }
    ...
    if action == "confirm_section":
        ...
        if confirmation_intent is not True:
            confirm_reason = _confirmation_block_reason(confirmation_intent)
        elif missing:
