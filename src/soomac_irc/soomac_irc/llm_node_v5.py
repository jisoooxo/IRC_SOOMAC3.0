...
from soomac_irc.agent_v5 import build_graph, build_selection_next_step
from soomac_irc.call_model_v5 import load_model, make_call_model, make_call_vlm, make_generate_reply
from soomac_irc.order_v5 import SECTION_LABELS, SECTION_ORDER, build_section_plan, new_order, next_section
...
class LLMNode(Node):
    def __init__(self):
        super().__init__("soomac_llm_node_v5")
    ...
    def _build_vlm_request(self, expected: str, camera_images: list) -> dict | None: # expected : 현재 로봇이 실제로 작업한 재료 이름.
        ...
        if expected in ("크림", "토마토", "오일"):
            ...
            return {
                ...
                "system_prompt": (
                    "너는 도시락 소스 작업을 확인하는 시각 판정기다. "
                    "이미지1은 선택한 소스의 참고 이미지이고 이미지2는 소스 작업 전 닫힌 도시락이다. "
                    "이후 이미지는 소스 작업 후 장면이다. "
                    f"검은 뚜껑이 계속 닫혀 있고 {expected} 소스가 그 위에 정상적으로 놓였으면 PASS이다. "
                    "소스가 없거나 다른 소스이거나 뚜껑이 열렸으면 FAIL이다. "
                    f"확신할 수 없으면 UNCERTAIN이다. {verdict_rule}"
                ),
                "user_text": (
                    f"작업 후 이미지 {len(camera_images)}장에서 "
                    f"{expected} 소스 작업이 실제로 완료됐는지 판정해."
                ),
            }
    ...
    def _process_pending_confirmation(self, user_text: str) -> bool:
        # 취소·현재 section 전체 제외의 재확인은 Tool 모델을 다시 호출하지 않고 처리한다.
        ...
        if pending["type"] == "cancel_order":
            # 이전 코드의 stale 상태가 들어와도 주문을 초기화하지 않음
            self._set_stt_enabled(True)
            reply = "전체 주문 취소는 지원하지 않아요. 현재 주문을 계속 진행할게요."

        elif pending["type"] == "refuse_section":
            ...
            if negative:
                ...
                self.skipped_sections[section] = pending["reason"]
    ...
    def _process_turn(self, user_text: str):
        ...
        # dislike section 제외만 다음 사용자 발화를 기다림
        ...
        if section_skip is not None and section_skip["needs_confirmation"]:
            self.awaiting_confirm = {
                "type": "refuse_section",
                "section": section_skip["section"],
                "reason": section_skip["reason"],
            }
    ...
    def _publish_status(self, work_state_override=None):
        ...
        if self.order_finished:
            ...
            if self.awaiting_confirm.get("type") == "cancel_order":
