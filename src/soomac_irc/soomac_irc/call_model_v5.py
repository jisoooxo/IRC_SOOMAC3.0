...
from soomac_irc.order_v5 import SAUCES, NOODLE_TYPES, ALL_TOPPINGS, AMOUNTS, TOPPING_AMOUNTS, RESTRICTION_REASONS, RESTRICTION_CATEGORY_ITEMS, SECTION_ORDER, missing_requirements
...
def parse_tool_call(raw: str) -> dict | None:
    ...
    # 메뉴 정본 검증은 order_v5에서 한 번 더 하므로 여기서는 Tool별 모양을 먼저 막음
    try:
    ...
    # restriction은 여러 개가 들어올 수 있으며 값 검증은 order_v5가 담당한다.
...
FREE_REPLY_SYSTEM = """
너는 파스타 주문 안내 직원이다.
제공된 현재 주문 상태와 사용자 발화를 사용해 자연스러운 한국어로 답한다.
