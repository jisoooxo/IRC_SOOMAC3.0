...
from soomac_irc.order_v4 import SAUCES, NOODLE_TYPES, ALL_TOPPINGS, AMOUNTS, TOPPING_AMOUNTS, RESTRICTION_REASONS, RESTRICTION_CATEGORY_ITEMS, SECTION_ORDER, missing_requirements
...
def parse_tool_call(raw: str) -> dict | None:
    ...
    # 메뉴 정본 검증은 order_v4에서 한 번 더 하므로 여기서는 Tool별 모양을 먼저 막음
    try:
    ...
    # restriction은 여러 개가 들어올 수 있으며 값 검증은 order_v4가 담당한다.
...
# respond Tool일 때만 사용하는 일반 답변 프롬프트
# 주문 변경·확정·안전 판단은 Python transaction에서 이미 끝난 상태임
FREE_REPLY_SYSTEM = """
너는 파스타 주문 안내 직원이다.
제공된 현재 주문 상태와 사용자 질문만 사용해서 자연스러운 한국어로 답한다.
주문을 변경하거나 로봇 작업을 시작·완료했다고 말하지 않는다.
확실하지 않은 내용은 추측하지 말고 다시 말해 달라고 요청한다.
JSON, 코드, 필드명, thought, 생각 과정은 출력하지 않는다.
...
def make_generate_reply(model, processor):
    ...
    def generate_reply(state: dict, user_text: str) -> str:
        ...
        return reply or "질문을 정확히 이해하지 못했어요. 다시 말씀해 주세요."
