...
def signed_delta_tick(raw_now, raw_home):
    return (
        int(raw_now) - int(raw_home) + 2048) % 4096 - 2048
...
class HardwareMotionControlNode(Node):
    ...
    def _build_phase_trajectory(self, phase, q_start, waypoints, class_name):
        ...
        raise ValueError(f'Unsupported trajectory phase: {phase}')
    ...
    def _connect_move_velocities(
        ...
        for index in range(len(trajectory) - 1):
            ...
            if (
                previous_segment['kind'] != 'move'
                or next_segment['kind'] != 'move'
            ):
                continue
    ...
    def _read_current_q(self):
        return self._raw_to_q(
            self._read_arm_positions()
        )
