...
class IRCKinematics:
    ...
    def solve_grip_place_pose(self, target, previous_q, place_yaw):
        q = self.solve_grip_place_point(target, previous_q)
    ...
    def solve_pose(self, target, previous_q, yaw, mode):
        ...
        q = self.solve_grip_pick_point(target, previous_q)
        q[5] = wrap_to_pi(yaw) + math.pi
        return q
    ...
    def grip_pick_candidates(self, active_indices, target, previous_q, q1):
        candidates = []
        ...
        for seed in self.seeds(previous_q, q1):
            ...
            def residual(active_q):
                ...
                position_error = transform[:3, 3] - target

                axis6 = transform[:3, 2]

                axis_error = axis6 - GRIP_TARGET_AXIS # 그리퍼가 수직으로 바닥을 보는 상태로 grip 하도록 한다. 재료를 안정적으로 잡기 위해서..
                ...
                return np.concatenate([
                    GRIP_POSITION_WEIGHT * position_error,
                    GRIP_AXIS_WEIGHT * axis_error,
                    GRIP_CONTINUITY_WEIGHT * joint_delta,
                ])
            ...
            axis6 = transform[:3, 2]
            axis6 = axis6 / np.linalg.norm(axis6)

            axis_dot = float(np.clip(np.dot(axis6, GRIP_TARGET_AXIS), -1.0, 1.0))
    ...
    def solve_grip_pick_point(self, target, previous_q):
        q1 = self.target_q1(target, previous_q[0])

        active_indices = np.array([1, 2, 3, 4], dtype=int)

        candidates = self.grip_pick_candidates(
            active_indices,
            target,
            previous_q,
            q1
    ...
    def solve_grip_place_point(self, target, previous_q):
        q1 = self.target_q1(target, previous_q[0])
        ...
        # place는 일단 목표 위치에 제대로 도달하는 해만 허용
