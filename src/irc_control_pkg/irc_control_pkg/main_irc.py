...
PLAN_CLASSES = {
    "sausage", "crab", "noodle_thick", "noodle_thin",
    "mushroom", "cheese", "pepperoncino", "onion",
    "sauce_cream", "sauce_oil", "sauce_tomato", "cover",
}
...
class MainNode(Node):
    ...
    def _control_home_callback(self, _msg):
        ...
        # cover까지 끝났으면 LLM에게 최종 완료 처리를 요청
    ...
    def _read_llm_plan(msg):
        ...
        if class_name not in PLAN_CLASSES:
            raise ValueError(f'지원하지 않는 LLM class입니다: {class_name}')
